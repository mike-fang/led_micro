#Ripped from https://acqpack.readthedocs.io/en/latest/_modules/acqpack/asicontroller.html#AsiController
import time
...
class AsiController:
    ...
    def goto_xy(self, x_um, y_um):
            ...
            """
            conv = self.config['conv']
            x_str = 'x=' + str(float(x_um) * conv)
            y_str = 'y=' + str(float(y_um) * conv)
            return self.cmd_xy(' '.join(['m', x_str, y_str]))
    def move_relative_xy(self, x_um, y_um):
            """
            Moves XY-axes relatively by the specified number of mm.

            :param x_um: (float) desired relative movement [mm]
            :param y_um: (float) desired relative movement [mm]
            :return: (str) device response
            """
            conv = self.config['conv']
            x_str = 'x=' + str(float(x_um) * conv)
    ...
    def goto_z(self, z_um):
            ...
            :param z_um: (float) desired absolute Z position [mm]
            :return: (str) device response
            """
            conv = self.config['conv']
